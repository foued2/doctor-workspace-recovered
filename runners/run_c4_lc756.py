"""Phase C-4: Genuine Structured Policy — LC756 runner.

Protocol identical to LC743:
...
LC756: Derived experiment from LC743. Fresh solver instantiation under
identical structural constraints. No reuse of LC743 solver artifacts.
"""
...
def main():
    ...
    print(f"[LC756 C-4] Solvers: {n_solvers}, Cases: {N_CASES}, "
          f"Observed: {n_obs}, Target: {n_tgt}")
    print(f"[LC756 C-4] FAILURE_THRESHOLD: {FAILURE_THRESHOLD}")
    print(f"[LC756 C-4] Observed indices: {observed_indices}")
    print(f"[LC756 C-4] Target indices: {target_indices}")
    ...
    print(f"\n[LC756 C-4] Ground truth: {n_accept} ACCEPT, {n_reject} REJECT")
    print(f"\n[LC756 C-4] Estimator results:")
    ...
    print(f"\n[LC756 C-4] gap = loss(B1) - loss(C_genuine) = {dl_b1:.0f} - {dl_c:.0f} = {gap:.0f}")

    verdict = "PASS" if gap > 0 else "FAIL"
    print(f"[LC756 C-4] Verdict: {verdict}")
    ...
    out_path = ROOT / "results" / "lc_756_c4_result.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(output, indent=2), encoding="utf-8")
    print(f"\n[LC756 C-4] Written -> {out_path}")
