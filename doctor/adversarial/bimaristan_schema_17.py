"""LC135 oracle evaluator implementing OracleEvaluatorContract."""
...
LC135_COMPLEXITY_CEILING = ComplexityCeiling(
    problem_id="lc135_candy",
    time_complexity="O(n!)",
    space_complexity="O(n)",
)


class LC135OracleCeilingError(RuntimeError):
    pass


class LC135OracleSymbolResolutionError(RuntimeError):
...
class LC135OracleExpressionError(ValueError):
    pass
...
class LC135OracleEvaluator:
    def __init__(self, max_n: int = 8) -> None:
        assert_valid_schema(LC135, registry=LC135_SYMBOL_REGISTRY)
        self.max_n = max_n
        self._registry = LC135_SYMBOL_REGISTRY

    def evaluate(self, surface: OracleEvaluationSurface) -> OracleResult:
        ratings = tuple(surface.candidate.raw_array)
        if len(ratings) > self.max_n:
            raise LC135OracleCeilingError(f"LC135 oracle ceiling exceeded: n > {self.max_n}")

        context: dict[str, Any] = {"ratings": list(ratings)}
        cache: dict[str, Any] = {}

        oracle_values: list[OracleSymbolValue] = []
        for entry in self._registry.entries:
            if entry.category is SymbolCategory.ORACLE_DEPENDENT:
                if not self._entry_inputs_available(entry.input_signature, context | cache):
                    continue
                try:
                    value = entry.compute(context | cache)
                except Exception as exc:
                    raise LC135OracleSymbolResolutionError(entry.name, exc) from exc
    ...
    def _visit(self, node: ast.AST, context: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            raise LC135OracleExpressionError(f"unknown symbol: {node.id}")
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            if name not in self._registry.names:
                raise LC135OracleExpressionError(f"unknown function: {name}")
        ...
        raise LC135OracleExpressionError(f"unsupported expression: {ast.dump(node)}")

    def _entry_inputs_available(self, input_signature: tuple[str, ...], context: dict[str, Any]) -> bool:
        return all(name in context for name in input_signature)
    ...
    def _operator_name(self, op: ast.cmpop) -> str:
        ...
        if isinstance(op, ast.NotIn):
            return "not_in"
        raise LC135OracleExpressionError(f"unsupported operator: {op}")
    ...
    def _compare(self, left: Any, operator_name: str, right: Any) -> bool:
        ...
        raise LC135OracleExpressionError(f"unsupported operator: {operator_name}")
...
def evaluation_surface(candidate, validation_predicates, generator_id: str, synthesized_input_id: str | None = None):
    return OracleEvaluationSurface(
        ...
        complexity_ceiling=LC135_COMPLEXITY_CEILING,
    )
