"""LC128 Stage 2.1 CLASS_C adversarial run."""
...
INSTANCES = 100
LC179_FLOOR = 0.16
OUTPUT_PATH = PROJECT_ROOT / "data" / "lc128_adversarial.json"


def lc128_input_order_consecutive_scan(nums: list[int]) -> int:
    """Wrong heuristic: longest +1 run in presented input order only."""
    if not nums:
        return 0
    best = 1
    current = 1
...
def _trace(instance_index: int) -> dict[str, Any]:
    ...
    if perturbation_valid:
        ...
        solver_output = lc128_input_order_consecutive_scan(copy.deepcopy(perturbed["nums"]))
    ...
    return {
        ...
        "solver_id": "lc128_input_order_consecutive_scan",
...
def run() -> dict[str, Any]:
    ...
    return {
        ...
        "lc179_floor": LC179_FLOOR,
        "delta_vs_lc179_floor": false_pass_rate - LC179_FLOOR,
    }
...
def main() -> int:
    ...
    print(
        ...
        f"delta_vs_lc179_floor={result['delta_vs_lc179_floor']:.2f}"
    )
