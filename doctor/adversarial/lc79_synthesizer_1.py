...
from doctor.adversarial.lc79_synthesizer import evaluate_lc79_candidates, generator_counts, synthesize_lc79_inputs


def main() -> None:
    batch = synthesize_lc79_inputs()
    evaluations = evaluate_lc79_candidates(batch)

    print("LC79 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for board, word in result.false_pass_inputs[:20]:
                print(f"      board={[list(row) for row in board]}, word={word!r}")
            if len(result.false_pass_inputs) > 20:
                print(f"      ... {len(result.false_pass_inputs) - 20} more")
        else:
