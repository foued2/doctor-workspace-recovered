...
from doctor.adversarial.lc33_synthesizer import (
    evaluate_lc33_candidates,
    generator_counts,
    synthesize_lc33_inputs,
)


def main() -> None:
    batch = synthesize_lc33_inputs()
    evaluations = evaluate_lc33_candidates(batch)

    print("LC33 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for nums, target in result.false_pass_inputs[:20]:
                print(f"      nums={list(nums)}, target={target}")
