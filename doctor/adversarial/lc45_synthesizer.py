...
from doctor.adversarial.lc45_synthesizer import (
    evaluate_lc45_candidates,
    generator_counts,
    synthesize_lc45_inputs,
)


def main() -> None:
    batch = synthesize_lc45_inputs()
    evaluations = evaluate_lc45_candidates(batch)

    print("LC45 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for nums in result.false_pass_inputs:
                print(f"      {list(nums)}")
        else:
