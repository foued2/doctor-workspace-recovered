...
from doctor.adversarial.lc560_synthesizer import (
    evaluate_lc560_candidates,
    generator_counts,
    synthesize_lc560_inputs,
)


def main() -> None:
    batch = synthesize_lc560_inputs()
    evaluations = evaluate_lc560_candidates(batch)

    print("LC560 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for nums, k in result.false_pass_inputs:
                print(f"      nums={list(nums)}, k={k}")
        else:
