"""LC560 oracle evaluator implementing OracleEvaluatorContract."""
...
LC560_COMPLEXITY_CEILING = ComplexityCeiling(
    problem_id="lc560_subarray_sum_equals_k",
    time_complexity="O(n^2)",
    space_complexity="O(1)",
)


class LC560OracleCeilingError(RuntimeError):
    pass


class LC560OracleSymbolResolutionError(RuntimeError):
...
class LC560OracleExpressionError(ValueError):
    pass
...
class LC560OracleEvaluator:
    ...
    def evaluate(self, surface: OracleEvaluationSurface) -> OracleResult:
        ...
        if len(nums) > self.max_n:
            raise LC560OracleCeilingError(f"LC560 oracle ceiling exceeded: n > {self.max_n}")

        context: dict[str, Any] = {"nums": nums, "k": k}
        cache: dict[str, Any] = {}
        ...
        for entry in self._registry.entries:
            if entry.category is SymbolCategory.ORACLE_DEPENDENT:
                ...
                try:
                    ...
                    raise LC560OracleSymbolResolutionError(entry.name, exc) from exc
    ...
    def _visit(self, node: ast.AST, context: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            raise LC560OracleExpressionError(f"unknown symbol: {node.id}")
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            if name not in self._registry.names:
                raise LC560OracleExpressionError(f"unknown function: {name}")
        ...
        raise LC560OracleExpressionError(f"unsupported expression: {ast.dump(node)}")
    ...
    def _operator_name(self, op: ast.cmpop) -> str:
        ...
        if isinstance(op, ast.NotIn):
            return "not_in"
        raise LC560OracleExpressionError(f"unsupported operator: {op}")
    ...
    def _compare(self, left: Any, operator_name: str, right: Any) -> bool:
        ...
        raise LC560OracleExpressionError(f"unsupported operator: {operator_name}")
...
def evaluation_surface(candidate, validation_predicates, generator_id: str, synthesized_input_id: str | None = None):
    return OracleEvaluationSurface(
        ...
        complexity_ceiling=LC560_COMPLEXITY_CEILING,
    )
