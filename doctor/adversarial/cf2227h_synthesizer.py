...
from doctor.adversarial.cf2227h_synthesizer import (
    evaluate_cf2227h_candidates,
    generator_counts,
    synthesize_cf2227h_inputs,
)


def main() -> None:
    batch = synthesize_cf2227h_inputs()
    evaluations = evaluate_cf2227h_candidates(batch)

    print("CF2227H Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for n, edges in result.false_pass_inputs[:20]:
                print(f"      n={n}, edges={list(edges)}")
