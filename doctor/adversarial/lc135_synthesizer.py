...
from doctor.adversarial.lc135_synthesizer import (
    evaluate_lc135_candidates,
    generator_counts,
    synthesize_lc135_inputs,
)


def main() -> None:
    batch = synthesize_lc135_inputs()
    evaluations = evaluate_lc135_candidates(batch)

    print("LC135 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for ratings in result.false_pass_inputs:
                print(f"      {list(ratings)}")
        else:
