...
"""CF2227H oracle evaluator implementing OracleEvaluatorContract."""
...
from doctor.adversarial.cf2227h_bimaristan import CF2227H
from doctor.adversarial.cf2227h_symbol_registry import CF2227H_SYMBOL_REGISTRY
...
CF2227H_COMPLEXITY_CEILING = ComplexityCeiling(
    problem_id="cf2227h_fallen_leaves",
    time_complexity="O(n^3)",
    space_complexity="O(n^2)",
)


class CF2227HOracleCeilingError(RuntimeError):
    pass


class CF2227HOracleSymbolResolutionError(RuntimeError):
...
class CF2227HOracleExpressionError(ValueError):
    pass
...
class CF2227HOracleEvaluator:
    def __init__(self, max_n: int = 12) -> None:
        assert_valid_schema(CF2227H, registry=CF2227H_SYMBOL_REGISTRY)
        self.max_n = max_n
        self._registry = CF2227H_SYMBOL_REGISTRY

    def evaluate(self, surface: OracleEvaluationSurface) -> OracleResult:
        n, raw_edges = surface.candidate.raw_array
        n = int(n)
        edges = [tuple(edge) for edge in raw_edges]
        if n > self.max_n:
            raise CF2227HOracleCeilingError(f"CF2227H oracle ceiling exceeded: n > {self.max_n}")

        context: dict[str, Any] = {"n": n, "edges": edges}
        cache: dict[str, Any] = {}
        ...
        for entry in self._registry.entries:
            try:
                ...
                raise CF2227HOracleSymbolResolutionError(entry.name, exc) from exc
    ...
    def _visit(self, node: ast.AST, context: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            raise CF2227HOracleExpressionError(f"unknown symbol: {node.id}")
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            if name not in self._registry.names:
                raise CF2227HOracleExpressionError(f"unknown function: {name}")
        ...
        raise CF2227HOracleExpressionError(f"unsupported expression: {ast.dump(node)}")
    ...
    def _operator_name(self, op: ast.cmpop) -> str:
        ...
        if isinstance(op, ast.Gt):
            return ">"
        raise CF2227HOracleExpressionError(f"unsupported operator: {op}")
    ...
    def _compare(self, left: Any, operator_name: str, right: Any) -> bool:
        ...
        if operator_name == ">":
            return left > right
        raise CF2227HOracleExpressionError(f"unsupported operator: {operator_name}")
...
def evaluation_surface(candidate, validation_predicates, generator_id: str, synthesized_input_id: str | None = None):
    return OracleEvaluationSurface(
        ...
        complexity_ceiling=CF2227H_COMPLEXITY_CEILING,
    )
