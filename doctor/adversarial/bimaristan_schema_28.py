"""LC45 oracle evaluator implementing OracleEvaluatorContract."""
...
class LC45OracleSymbolResolutionError(RuntimeError):
...
class LC45OracleExpressionError(ValueError):
    pass
...
class LC45OracleEvaluator:
    def __init__(self, max_n: int = 15) -> None:
        ...
        self.max_n = max_n
        self._registry = LC45_SYMBOL_REGISTRY

    def evaluate(self, surface: OracleEvaluationSurface) -> OracleResult:
        input_array = tuple(surface.candidate.raw_array)
        if len(input_array) > self.max_n:
            raise OracleCeilingError(f"LC45 oracle ceiling exceeded: n > {self.max_n}")

        context: dict[str, Any] = {"nums": list(input_array)}
        cache: dict[str, Any] = {}
        ...
        for entry in self._registry.entries:
            if entry.category is SymbolCategory.ORACLE_DEPENDENT:
                ...
                try:
                    ...
                    raise LC45OracleSymbolResolutionError(entry.name, exc) from exc
    ...
    def _visit(self, node: ast.AST, context: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            raise LC45OracleExpressionError(f"unknown symbol: {node.id}")
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            if name not in self._registry.names:
                raise LC45OracleExpressionError(f"unknown function: {name}")
        ...
        raise LC45OracleExpressionError(f"unsupported expression: {ast.dump(node)}")

    def _entry_inputs_available(self, input_signature: tuple[str, ...], context: dict[str, Any]) -> bool:
        for name in input_signature:
            if name.endswith("?"):
                continue
