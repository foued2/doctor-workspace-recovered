...
def _candidate_space(manifold_id: str):
    seeds = {
        ...
        "left_pass_plateau_cliff": (
            [10, 8, 8, 8, 3, 1],
            [12, 9, 7, 7, 7, 2, 1],
            [15, 12, 10, 10, 10, 4, 1],
        ),
        "right_pass_plateau_cliff": (
            [1, 3, 8, 8, 8, 10, 12],
            [1, 2, 7, 7, 7, 9, 12],
            [1, 4, 10, 10, 10, 12, 15],
        ),
...
def _apply_constraint_phase_crossing(manifold_id: str, accepted: list[dict[str, Any]]) -> dict[str, Any] | None:
    if manifold_id in {"left_pass_plateau_cliff", "right_pass_plateau_cliff"}:
        pre_divergence_rate = _both_directions_required_rate(accepted)
        crossed = [_inject_plateau_cliff(context) for context in accepted]
...
def _inject_plateau_cliff(context: dict[str, Any]) -> dict[str, Any]:
    ratings = list(context["ratings"])
    n = len(ratings)
    if n < 5:
        return context
    mid = n // 2
    plateau_start = max(1, mid - 1)
    plateau_end = min(n - 2, mid + 1)
    plateau_value = (ratings[plateau_start - 1] + ratings[plateau_end + 1]) // 2
    result = list(ratings)
    for i in range(plateau_start, plateau_end + 1):
        result[i] = plateau_value
    cliff_pos = plateau_end + 1
    if cliff_pos < n:
        drop = max(ratings) - min(ratings)
        result[cliff_pos] = max(plateau_value - drop, min(ratings))
