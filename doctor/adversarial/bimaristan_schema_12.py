...
def test_short_left_hidden_partner_surface_is_satisfiable_after_schema_fix():
    constraints = (
        ...
        RelationConstraint("hidden_partner_height", ">", "right_height"),
    ...
    batch = LC11Synthesizer(seed=42).synthesize(surface)
    assert len(batch.accepted_candidates) >= 10
    assert len(batch.rejected_candidates) / (len(batch.accepted_candidates) + len(batch.rejected_candidates)) < 0.2
    assert all(
        evaluate_constraint(constraint, candidate_context(surface.generator_id, candidate.raw_array))
        for candidate in batch.accepted_candidates
        for constraint in constraints
    )


def test_nonrealizable_manifold_gate_skips_before_generation():
    surface = GenerationSurface("synthetic_nonrealizable", (), (GenerationStrategy.INTERIOR_SPIKE, GenerationStrategy.PLATEAU))
    synth = LC11Synthesizer(
        seed=42,
        manifold_index={"synthetic_nonrealizable": ("synthetic_manifold", "NONREALIZABLE", ())},
    )
    batch = synth.synthesize(surface)
    assert batch.accepted_candidates == ()
    assert batch.rejected_candidates == ()
    assert synth.skipped[0].manifold_id == "synthetic_manifold"
