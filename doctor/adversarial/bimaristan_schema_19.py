...
"""LC20 oracle evaluator implementing OracleEvaluatorContract."""
...
from doctor.adversarial.lc20_bimaristan import LC20
...
LC20_COMPLEXITY_CEILING = ComplexityCeiling(
    problem_id="lc20_valid_parentheses",
    time_complexity="O(n)",
    space_complexity="O(n)",
)


class LC20OracleCeilingError(RuntimeError):
    pass


class LC20OracleSymbolResolutionError(RuntimeError):
...
class LC20OracleExpressionError(ValueError):
    pass
...
class LC20OracleEvaluator:
        ...
        for entry in self._registry.entries:
            if entry.category is SymbolCategory.ORACLE_DEPENDENT:
                try:
                    ...
                    raise LC20OracleSymbolResolutionError(entry.name, exc) from exc
    ...
    def _visit(self, node: ast.AST, context: dict[str, Any]) -> Any:
        ...
        if isinstance(node, ast.Name):
            ...
            raise LC20OracleExpressionError(f"unknown symbol: {node.id}")
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
            ...
            if name not in self._registry.names:
                raise LC20OracleExpressionError(f"unknown function: {name}")
        ...
        raise LC20OracleExpressionError(f"unsupported expression: {ast.dump(node)}")
    ...
    def _operator_name(self, op: ast.cmpop) -> str:
        ...
        if isinstance(op, ast.Gt):
            return ">"
        raise LC20OracleExpressionError(f"unsupported operator: {op}")
    ...
    def _compare(self, left: Any, operator_name: str, right: Any) -> bool:
        ...
        if operator_name == ">":
            return left > right
        raise LC20OracleExpressionError(f"unsupported operator: {operator_name}")
...
def evaluation_surface(candidate, validation_predicates, generator_id: str, synthesized_input_id: str | None = None):
    return OracleEvaluationSurface(
        ...
        complexity_ceiling=LC20_COMPLEXITY_CEILING,
    )
