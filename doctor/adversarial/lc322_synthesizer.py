...
from doctor.adversarial.lc322_synthesizer import (
    evaluate_lc322_candidates,
    generator_counts,
    synthesize_lc322_inputs,
)


def main() -> None:
    batch = synthesize_lc322_inputs()
    evaluations = evaluate_lc322_candidates(batch)

    print("LC322 Bimaristan native run")
    ...
    for result in evaluations:
        ...
        if result.false_pass_inputs:
            print("    FALSE PASS:")
            for coins, amount in result.false_pass_inputs:
                print(f"      coins={list(coins)}, amount={amount}")
        else:
