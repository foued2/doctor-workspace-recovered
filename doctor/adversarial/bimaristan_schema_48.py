...
class ConstraintEvaluationError(RuntimeError):
    def __init__(self, constraint: GeometryConstraint, original: Exception) -> None:
        self.constraint = constraint
        self.original = original
        super().__init__(f"{_constraint_id(constraint)}: {type(original).__name__}: {original}")


class SynthesisProblemMismatchError(RuntimeError):
    def __init__(self, expected_problem_id: str, actual_problem_id: str) -> None:
        self.expected_problem_id = expected_problem_id
        self.actual_problem_id = actual_problem_id
        super().__init__(
            f"LC11Synthesizer cannot synthesize surface for {actual_problem_id!r}; "
            f"expected {expected_problem_id!r}"
        )
...
class LC11Synthesizer:
    def __init__(
        ...
        assert_valid_schema(LC11, registry=LC11_SYMBOL_REGISTRY)
        self.seed = seed
    ...
    def synthesize(self, surface: GenerationSurface) -> SynthesisBatch:
        self._assert_surface_problem(surface)
    ...
    def _assert_surface_problem(self, surface: GenerationSurface) -> None:
        expected = LC11.problem_structure.problem_id
        if surface.problem_id != expected:
            raise SynthesisProblemMismatchError(expected, surface.problem_id)
        ...
        for constraint in constraints:
            try:
                ...
                if self.strict_constraints:
                    raise ConstraintEvaluationError(constraint, exc) from exc
                return constraint
        return None
