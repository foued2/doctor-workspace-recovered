...
evaluator geometry — it cannot be embedded into an external solver space. DCG: GDC-FAILING (shared
lineage with CSSE v2 probes).
...
- LC743: all estimators converge to identical predictions (no behavioral diversity); external
  support insufficient (n=1, EVC-FAILING)
...
DCG decomposes into two independent predicates:

    DCG(S,P) = GDC(S,P) ∧ EVC(S,P)

### GDC — Generation Domain Constraint

GDC tests whether solver source S is independent of the CSSE v2 generation system. GDC is defined
purely on provenance metadata, not on behavioral similarity, output overlap, or interface
properties. A source S for problem P satisfies GDC iff:

1. **Provenance disjointness.** S ∩ CSSE_generators = ∅, defined at generation time. S was not
   generated by the CSSE mutation engine, any system sharing its codebase, or any system that
   received CSSE probe families, failure taxonomy, or mutation operators as input. This is
   verified by generation-time metadata (author, tool, commit hash, date), not by output
   comparison.

2. **No transformation lineage overlap.** No solver in S is reachable via mutation graph from
   CSSE seeds. If S was produced by applying transformations to a CSSE-generated seed, GDC
   fails regardless of how many transformations were applied.

3. **Interface irrelevance.** Interface differences (argument count, class structure, import
   patterns) are not used as evidence for or against GDC. GDC is determined at generation time,
   not inferred from structural similarity after the fact.

GDC is falsifiable: a provenance audit can determine whether S ∩ CSSE_generators = ∅. It is not
inferred from P(D), CE rate, failure variance, or any observed statistic.

### EVC — Evaluation Validity Condition (B-Domain)

EVC tests whether source S can be evaluated under a unified protocol. EVC operates on behavioral
equivalence classes (B-domain), not on syntactic representation (R-domain). A source S for problem
P satisfies EVC iff:

1. **B-equivalence completeness.** All solvers in S can be classified into B-equivalence classes
   under the evaluation oracle E_P. If B-classification is incomplete (e.g., due to CE preventing
   behavioral testing), EVC fails.

2. **Unified evaluator.** A single evaluator E_P is applied across all B-equivalence classes:

       E_P^{class_1} ≡ E_P^{class_2}

   If evaluator behavior depends on B-class membership, EVC fails.

3. **Endogenous failure.** Failures are generated by evaluation:

       F(x) := 1[E_P(s, x) ≠ y_true]

   and are not dataset-encoded labels (e.g., `is_pass`). If ground-truth failure is pre-attached,
   the source is descriptive-only.

4. **Sufficient B-domain sample.** The source contains enough B-equivalence classes to estimate
   P(D) with non-degenerate variance. A minimum of n ≥ 30 valid B-classes is required. Below
   this threshold, P(D) exists but is not estimable with useful precision.
...
Solver identity is a two-level object. The system defines two orthogonal equivalence relations:
R-equivalence (representation layer) and B-equivalence (behavioral layer). These form a product
structure, not a choice.

### R-Equivalence (Representation Equivalence)

Two solvers s_1, s_2 are R-equivalent (s_1 ≡_R s_2) iff they differ only by admissible syntactic
transformations. R-equivalence is purely structural normalization that removes syntax noise without
altering semantic content.

**Admissible R-transformations (closed under composition):**

| Transformation | Domain | Codomain | Condition |
|----------------|--------|----------|-----------|
| T1: class extraction | `class S: def f(self, x)` | `def f(x)` | method body unchanged |
| T2: argument unification | `def f(coins, amount)` | `def f(solver_input)` | unpacking is injective |
| T3: import normalization | `from m import S` | `from m import S` | path normalization only |
| T4: indentation normalization | any indentation | consistent 4-space | semantic equivalence preserved |
| T5: comment removal | any comments | no comments | no semantic content |
| T6: dead code elimination | unreachable code | removed | no reachable behavior changed |

**Closure property:** If T_i and T_j are admissible, then T_j ∘ T_i is admissible. The set of
R-transformations forms a monoid under composition.

**R-equivalence is verified by:** structural comparison after normalization, not by behavioral
testing. Two solvers are R-equivalent iff their normalized ASTs are isomorphic (modulo variable
names).

### B-Equivalence (Behavioral Equivalence)

Two solvers s_1, s_2 are B-equivalent (s_1 ≡_B s_2) iff for all admissible inputs in the test
distribution D_P, they produce identical outputs under the evaluation oracle E_P:

    s_1 ≡_B s_2 ⟺ ∀x ∈ D_P: E_P(s_1, x) = E_P(s_2, x)

B-equivalence is verified by behavioral testing, not by structural comparison. Two solvers can be
B-equivalent without being R-equivalent (different syntax, same behavior) or R-equivalent without
being B-equivalent (same syntax after normalization, different behavior due to bugs).

### Product Structure

Solver identity is the product:

    identity(s) = [R-class(s), B-class(s)]

where R-class(s) is the equivalence class under ≡_R and B-class(s) is the equivalence class under
≡_B. The evaluation invariant is:

    E_P(s, x) depends only on B-class(s), not on R-class(s)

This means: normalization (R-layer) must be semantics-preserving. If two solvers are R-equivalent,
they must be B-equivalent. Violation of this property means the R-normalizer is not
representation-preserving.

### Layer Separation for DCG/EVC

The bi-level system separates DCG constraints:

- **GDC** operates on R-domain: provenance disjointness is a property of the generation process,
...
layer was an R-normalizer; its CE failures were R-layer failures, not B-layer failures.

### R-Coverage Condition (Missing Invariant)

B-equivalence is well-defined mathematically, but its empirical estimation is R-dependent: the
R-filter determines which solvers enter B-evaluation. If R-filtering systematically eliminates
certain B-equivalence classes from observation, then B-estimates are biased by R-selection.

**R-coverage condition:** For B-equivalence to be statistically transportable across solver
ecosystems:

    ∀ B-class(b), the induced measure over D_P must be invariant under R-selection bias

Operational form: no R-class may systematically eliminate a B-equivalence class from observation.
If an R-class (e.g., class-based solvers) has higher CE rate than other R-classes, then
B-equivalence estimates are biased toward R-classes that survive normalization.

**Current status:** R-coverage is violated for LC79/LC743 external (class-based solvers have
higher CE, reducing B-observable sample). R-coverage is partially satisfied for LC322 external
(mixed R-classes, but sufficient B-sample after filtering).

**Consequence:** EVC is defined but not yet statistically transportable across solver ecosystems.
DCG becomes a coverage-aware gate, not just a provenance/evaluation split.

### R as Sampling Operator

R-equivalence is not merely normalization. It is an experimental sampling operator over B-space:
R determines which parts of B-space are observable. This means:

- CE rate is an R-space property (normalization failure), not a B-space property
- P(D) estimates are conditional on R-selection: P(D | R-filter, problem class)
...
All four CSSE v2 sources are GDC-FAILING: they share mutation lineage with the CSSE v2 generator.
They satisfy EVC (canonical interface, unified evaluator, endogenous failures, sufficient sample)
but fail GDC. CSSE v2 is valid for internal measurement (P(D | mutation distribution)) but not
...
### DCG Classification (Pre-Result)

| (Source, Problem) | GDC | EVC | DCG | Notes |
|-------------------|-----|-----|-----|-------|
| External, LC322   | pass | pass | VALID | independent provenance, n=38 ≥ 30 |
| External, LC79    | pass | EVC-FAILING | EVC-FAILING | n=1 < 30 |
| External, LC743   | pass | EVC-FAILING | EVC-FAILING | n=1 < 30 |

GDC passes for all external sources: they are independent of CSSE v2 mutation lineage. EVC fails
for LC79 and LC743 due to insufficient sample size (n=1 < 30).
...
The only valid DCG comparison:

| Source | P(D) | n | DCG |
|--------|------|---|-----|
| CSSE v2 (internal mutation) | 0.019 | 317 | GDC-FAILING |
| External (human+GPT) | 0.053 | 38 | VALID |

CSSE v2 is GDC-FAILING (shared mutation lineage), so it is not a valid external comparator.
The comparison is between External P(D) and the internal CSSE v2 P(D), with the caveat that
CSSE v2 is an internal measure for contrast only. External P(D) is higher than CSSE v2 P(D).
Both are low. The confidence intervals have not been formally compared (external CI not yet
computed), but the point estimates are in the same order of magnitude.
...
### LC79/LC743 External Results (EVC-Failing)

LC79: 1 valid solver, P(D)=0.000. EVC-FAILING (n=1 < 30).

LC743: 1 valid solver, P(D)=0.000. EVC-FAILING (n=1 < 30).

These are labeled EVC-FAILING. No cross-domain P(D) inference is permitted.
...
DCG = GDC ∧ EVC ∧ R-coverage. GDC is R-domain (provenance). EVC is B-domain (evaluation).
R-coverage is sampling invariance (no R-class eliminates B-classes from observation).

| (Source, Problem) | GDC | EVC (B-domain) | R-coverage | DCG | Notes |
|-------------------|-----|----------------|------------|-----|-------|
| CSSE v2, LC322    | GDC-FAILING | pass | pass | GDC-FAILING | generated by CSSE mutation engine |
| CSSE v2, LC3946   | GDC-FAILING | pass | pass | GDC-FAILING | generated by CSSE mutation engine |
| CSSE v2, LC79     | GDC-FAILING | pass | pass | GDC-FAILING | generated by CSSE mutation engine |
| CSSE v2, LC743    | GDC-FAILING | pass | pass | GDC-FAILING | generated by CSSE mutation engine |
| External, LC322   | pass | pass | partial | VALID | independent provenance, n=38, mixed R-classes |
| External, LC79    | pass | EVC-FAILING | violated | EVC-FAILING | n=1, class-based CE bias |
| External, LC743   | pass | EVC-FAILING | violated | EVC-FAILING | n=1, class-based CE bias |
| LC3946, hand-coded| pass | pass | pass | VALID | hand-coded, not CSSE-generated; structurally excluded |

GDC-FAILING means the source shares mutation lineage with CSSE v2; it is valid for internal
measurement but not for cross-domain comparison. EVC-FAILING means B-domain evaluation constraints
are not met (insufficient B-equivalence classes). VALID means both GDC and EVC hold.

### The Only Valid Cross-Domain Comparison

LC322 is the only (source, problem) pair where External satisfies DCG. CSSE v2 is GDC-FAILING
(shared mutation lineage), so the cross-domain comparison is between External and the CSSE v2
internal measure — but CSSE v2 is not a valid external comparator. The comparison is therefore
between External P(D) and the internal CSSE v2 P(D), with the caveat that CSSE v2 is
GDC-FAILING.
...
order of magnitude. The comparison is informative but CSSE v2 is not a DCG-valid external
anchor — it is an internal measure for contrast only.
...
perturbation survival. However, LC3946 is structurally excluded from external comparison: it is a
synthetic evaluator geometry, not a real-world solver domain. The hand-coded LC3946 solvers pass
GDC (they are not CSSE-generated), but the problem itself cannot be embedded into an external
solver space. LC3946 is valid for internal measurement but outside the domain of cross-domain
...
insufficient external support (n=1, EVC-FAILING). These failures constrain interpretation: the
C_genuine/B1 divergence appears when the solver population has failures that split across multiple
probe families under C_genuine's rule but are treated uniformly by B1.
...
vs 1.0) and partially robust (10/11 perturbation survival). However, LC3946 is structurally
excluded from external comparison: it is a synthetic evaluator geometry, not a real-world solver
domain. The hand-coded solvers pass GDC, but the problem itself is non-externalizable.
...
becomes superior. LC322 is the only problem class where External satisfies DCG. The cross-domain
comparison is valid but underpowered (external n=38). CSSE v2 is GDC-FAILING, so it serves as an
internal contrast, not a valid external comparator.

The LC45 and LC743 results demonstrate boundary conditions where the mechanism fails. LC743
additionally has insufficient external support (n=1, EVC-FAILING), so no cross-domain inference is
...
provenance and interface properties, not on observed outcomes, so it cannot be used post-hoc to
classify results as valid or invalid. The single valid external anchor (LC322) is underpowered.
A larger external corpus would strengthen the external measure but is outside the scope of this
paper.
...
The primary result is positive but structurally excluded from external comparison. On LC3946
(poset-based lattice), C_genuine achieves decision_loss=0.0 versus B1's 1.0, strictly improving
over all non-degenerate baselines. The advantage survives 10 of 11 perturbation conditions.
However, LC3946 is a synthetic evaluator geometry — it cannot be embedded into an external solver
...
(n=1, EVC-FAILING). No cross-domain inference is permitted for LC743.
