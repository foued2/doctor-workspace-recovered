"""SVD analysis for LC322 v2 solver population (simplified)."""
import json
import math
import random
import sys
...
def evaluate_v2_solvers(to_input, oracle_fn, style):
    ...
    obs, tgt = load_observed_target_split("lc322")
    
    # Load v2 solvers
    v2_dir = ROOT.parent / "experiments" / "frozen_taxonomy_lc322_v2" / "solvers"
    ...
    for solver_path in solver_files:
        ...
        # Evaluate on probes (with timeout protection)
        results = {}
        for pid in obs:
...
def run_analysis():
    ...
    print("="*70)
    print("  LC322 V2 POPULATION SVD ANALYSIS")
    print("="*70)
    
    # Load data
    probes = load_probes("lc322")
    obs, tgt = load_observed_target_split("lc322")
    gt = load_ground_truth_from_json("lc322")
    
    print(f"\n  Probes: {len(obs)}")
    
    # Evaluate v2 solvers
    print(f"\n  Evaluating v2 solvers...")
    solver_evals = evaluate_v2_solvers(lc322_to_input, lc322_oracle, "single")
    ...
    if orig_path.exists():
        ...
        orig_lc322 = orig_data.get("lc322", {})
        orig_sv = orig_lc322.get("singular_values", [])
        orig_dim = orig_lc322.get("intrinsic_dim")
    ...
    out_path = ROOT / "results" / "svd_analysis_lc322_v2.json"
